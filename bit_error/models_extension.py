...
class conv_with_bias(nn.Module):
    ...
    def forward(self, x):
        ...
        for batch in range(0,x.shape[0]):
            temp = convol_C(temp.cpu().astype(np.float32),weights.detach().cpu().numpy().astype(np.float32),kernel_shape.cpu().astype(np.float32),x[batch,:,:,:].cpu().astype(np.float32),x_shape.cpu().astype(np.float32),outplanes,h_out,w_out,stride,kernel_size,bias.detach().cpu().numpy().astype(np.float32))
            output[batch,:,:,:] =(torch.from_numpy(temp)).view(1,self.outplanes,h_out,w_out)
        return output
